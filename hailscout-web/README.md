...
NEXT_PUBLIC_API_BASE_URL=https://api.hailgps.com
NEXT_PUBLIC_TILES_BASE_URL=https://tiles.hailgps.com
...
The frontend calls `https://api.hailgps.com` endpoints via the `apiClient` wrapper in `src/lib/api.ts`. The wrapper:
...
- Vector tiles served at `https://tiles.hailgps.com/swaths/{z}/{x}/{y}.pbf`
