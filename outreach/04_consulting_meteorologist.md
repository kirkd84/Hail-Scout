# Consulting Meteorologist — Outreach (Send Month 3, not yet)

**To:** AMS CCM directory — forensic meteorology specialty
**From:** kirk@rooftechnologies.com
**Subject:** Part-time consulting met for hail SaaS — inquiry

---

Hi [Name],

I'm Kirk, founder of Hail GPS, a storm-intelligence platform for roofing contractors launching this year. We're looking for a certified consulting meteorologist for retained part-time work starting month 4-5.

The role:
- Review a small volume of "legal-tier" Hail Impact Reports each month (the ones destined for insurance disputes or court)
- Occasionally consult on QC of our AI-generated storm scoring
- ~5-15 hours per month retainer, scaling with customer growth

Pay: competitive hourly or monthly retainer, your choice. Fully remote.

If you'd be open to a 20-minute chat, I'd love to walk through what we're building and see if it's a fit.

Best,
Kirk
Founder, Hail GPS
kirk@rooftechnologies.com

---

## Internal notes (don't send)

- **Don't send Week 1** — Month 3, after we have an actual product to show. Sending too early is amateur.
- **Source list:** AMS CCM directory + LinkedIn ("forensic meteorologist" + "consulting meteorologist"). Aim to message 10.
- **Why this matters:** PRD §1.3 calls out "AI + on-demand met" as our positioning vs. HailTrace/IHM's full-met-review. Risk §5 lists MESH-accuracy complaints as high-likelihood — having a CCM on retainer is the mitigation.
- **Budget envelope:** $2-5K/month retainer at first; scale with customer count. Build into pricing for the "legal tier" report add-on.
